...
def print_outtable(results_dict, grid_size):
    ...
    table.column_headers = ["Simulation Number", "City Grid Size", "Number Pedestrians", "Top Location Node",
                            "Number of Times Node in a Pedestrian Path",
                            "Highest Number of Pedestrian Collisions for Node"]
    for result in results_dict:
        table.append_row([str(result), str(grid_size), str(results_dict[result]["Pedestrians"]),
                          str(results_dict[result]["Top_Location"]), str(results_dict[result]["Number_Intersections"]),
...
def main():
    ...
    while num_simuls > 0:  # Run simulations and record results
        ...
        if results_list:
            top_collision_count_list = sorted(results_list[0][1]["position_collisions"].items(), key=lambda x: x[1],
                                              reverse=True)
            top_collision_count = top_collision_count_list[0][1]
            simulation_summary[num_simuls] = {
    ...
    top_place = sorted(simulation_summary.items(), key=lambda x: x[1]["Number_Intersections"], reverse=True)[0]
